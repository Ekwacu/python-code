...
fig.subplots_adjust(hspace=0.5, wspace=0.05, top=0.95,
                    bottom=0.2, left=0.40, right=0.975)


nrows = 3  # number of plot rows
ncols = 2  # number of plot columns

#DATA_PATH = "G:/Folder_2022/Research/Hotspells_spatial/extracted/Hotspell-csv/correlation_2/"
DATA_PATH= "F:/Folder_2022/Research/Hotspells_spatial/cor_ssts_hotspells/Hotspell_tmin-15-cor/"
...
for index, region in enumerate(regions):
    ...
    dset = xr.open_dataset(DATA_PATH + region + '_cor_sst_hotspell_tmin_15.nc')
    ...
    dset.cor.plot(cmap='RdBu', vmin=-1.0, vmax=1.0,
                  cbar_kwargs={'orientation': 'vertical', 'label': ''})
    plt.contourf(lon, lat, dset.cor_sig, hatches=['xxxx'], alpha=0)
    plot_extras(ax)
    ax.set_title(region, fontsize=12, fontweight='bold')

plt.savefig('F:/Folder_2022/Research/cor_plots/Tmin_15_cor_sst_hotspell.png',
